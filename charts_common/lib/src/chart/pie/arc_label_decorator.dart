...
import '../../common/text_element.dart' show MaxWidthStrategy, TextDirection, TextElement;
...
class ArcLabelDecorator<D> extends ArcRendererDecorator<D> {
  ...
  static final _defaultInsideLabelStyle = new TextStyleSpec(fontSize: 12, color: Color.white);
  static final _defaultOutsideLabelStyle = new TextStyleSpec(fontSize: 12, color: Color.black);
  static final _defaultLeaderLineStyle = new ArcLabelLeaderLineStyleSpec(
      length: 20.0, thickness: 1.0, color: StyleFactory.style.arcLabelOutsideLeaderLine);
  ...
  ArcLabelDecorator(
      ...
      : insideLabelStyleSpec = insideLabelStyleSpec ?? _defaultInsideLabelStyle,
        outsideLabelStyleSpec = outsideLabelStyleSpec ?? _defaultOutsideLabelStyle,
        leaderLineStyleSpec = leaderLineStyleSpec ?? _defaultLeaderLineStyle;

  @override
  void decorate(ArcRendererElementList<D> arcElements, ChartCanvas canvas, GraphicsFactory graphicsFactory,
      {@required Rectangle drawBounds, @required double animationPercent, bool rtl = false}) {
    ...
    final insideLabelStyle = _getTextStyle(graphicsFactory, insideLabelStyleSpec);
    final outsideLabelStyle = _getTextStyle(graphicsFactory, outsideLabelStyleSpec);

    // Track the Y position of the previous outside label for collision
    // detection purposes.
    num previousOutsideLabelY;
    bool previousLabelLeftOfChart;
    List<double> centerAngles = new List();
    int i = 0;
    for (var element in arcElements.arcs) {
      ...
      final label0 = label.split(';')[0];
      final label1 = label.split(';')[1];
      ...
      final datumInsideLabelStyle = _getDatumStyle(
          element.series.insideLabelStyleAccessorFn, datumIndex, graphicsFactory,
          defaultStyle: insideLabelStyle);
      final datumOutsideLabelStyle = _getDatumStyle(
          element.series.outsideLabelStyleAccessorFn, datumIndex, graphicsFactory,
      ...
      double centerAngle = element.startAngle + (arcAngle / 2);
      double trueCenterAngle = centerAngle;
      if (i > 0) {
      ...
      final centerRadius = arcElements.innerRadius + ((arcElements.radius - arcElements.innerRadius) / 2);
      i++;
      final innerPoint = new Point<double>(arcElements.center.x + arcElements.innerRadius * cos(centerAngle),
          arcElements.center.y + arcElements.innerRadius * sin(centerAngle));

      final outerPoint = new Point<double>(arcElements.center.x + arcElements.radius * cos(centerAngle),
      ...
      final insideArcWidth = (min((((arcAngle * 180 / pi) / 360) * (2 * pi * centerRadius)).round(),
      ...
      final outsideArcWidth = ((drawBounds.width / 2) - bounds.width - totalPadding - leaderLineLength).round();

      final labelElement = graphicsFactory.createTextElement(label)..maxWidthStrategy = MaxWidthStrategy.ellipsize;
      final labelElement0 = graphicsFactory.createTextElement(label0)..maxWidthStrategy = MaxWidthStrategy.ellipsize;
      final labelElement1 = graphicsFactory.createTextElement(label1)..maxWidthStrategy = MaxWidthStrategy.ellipsize;
      ...
      if (calculatedLabelPosition == ArcLabelPosition.auto) {
        ...
        calculatedLabelPosition =
            (insideArcWidth >= outsideArcWidth || labelElement.measurement.horizontalSliceWidth < insideArcWidth)
      ...
      if (calculatedLabelPosition == ArcLabelPosition.inside) {
        ...
        labelElement.maxWidth = outsideArcWidth + 400;
        labelElement0.textStyle = datumOutsideLabelStyle;
        labelElement0.maxWidth = outsideArcWidth + 400;
        labelElement1.textStyle = datumOutsideLabelStyle;
        labelElement1.maxWidth = outsideArcWidth + 400;
      }
      ...
      if (labelElement.maxWidth > 0) {
        ...
        if (calculatedLabelPosition == ArcLabelPosition.inside) {
          ...
          final l = _drawOutsideLabel(canvas, drawBounds, arcElements, labelElement0, labelElement1, centerAngle,
              previousOutsideLabelY, previousLabelLeftOfChart, trueCenterAngle);
  ...
  TextStyle _getTextStyle(GraphicsFactory graphicsFactory, TextStyleSpec labelSpec) {
  ...
  TextStyle _getDatumStyle(AccessorFn<TextStyleSpec> labelFn, int datumIndex, GraphicsFactory graphicsFactory,
      {TextStyle defaultStyle}) {
    final styleSpec = (labelFn != null) ? labelFn(datumIndex) : null;
    return (styleSpec != null) ? _getTextStyle(graphicsFactory, styleSpec) : defaultStyle;
  }

  /// Draws a label inside of an arc.
  void _drawInsideLabel(
      ChartCanvas canvas, ArcRendererElementList<D> arcElements, TextElement labelElement, double centerAngle) {
    // Center the label inside the arc.
    final labelRadius = arcElements.innerRadius + (arcElements.radius - arcElements.innerRadius) / 2;

    final labelX = (arcElements.center.x + labelRadius * cos(centerAngle)).round();

    final labelY = (arcElements.center.y + labelRadius * sin(centerAngle) - insideLabelStyleSpec.fontSize / 2).round();
  ...
  List _drawOutsideLabel(
      ...
      TextElement labelElement0,
      TextElement labelElement1,
      double centerAngle,
      num previousOutsideLabelY,
      bool previousLabelLeftOfChart,
      double trueCenterAngle) {
    final labelRadius = arcElements.radius + leaderLineStyleSpec.length / 2;

    final labelPoint = new Point<double>(
        arcElements.center.x + labelRadius * cos(centerAngle), arcElements.center.y + labelRadius * sin(centerAngle));
    ...
    var labelX = labelLeftOfChart ? (labelPoint.x - labelPadding).round() : (labelPoint.x + labelPadding).round();

    // Shift the label up by the size of the font.
    final labelY = (labelPoint.y - outsideLabelStyleSpec.fontSize - 3).round();
    final secondY = (labelPoint.y + 2).round();
    // Outside labels should flow away from the center of the chart
    labelElement0.textDirection = labelLeftOfChart ? TextDirection.ltr : TextDirection.rtl;
    labelElement1.textDirection = labelLeftOfChart ? TextDirection.ltr : TextDirection.rtl;

    // Skip this label if it collides with the previously drawn label.
    if (_detectOutsideLabelCollision(labelY, labelLeftOfChart, previousOutsideLabelY, previousLabelLeftOfChart)) {
      return null;
    }

    if (showLeaderLines) {
      final tailX = _drawLeaderLine(
          canvas, labelLeftOfChart, labelPoint, arcElements.radius, arcElements.center, centerAngle,
          trueCenterAngle: trueCenterAngle);

      // Shift the label horizontally by the length of the leader line.
      labelX = (labelX + tailX).round();

      labelElement0.maxWidth = (labelElement0.maxWidth - tailX).round();
      labelElement1.maxWidth = (labelElement1.maxWidth - tailX).round();
    }

    canvas.drawText(labelElement0, labelX, labelY);
    canvas.drawText(labelElement1, labelX, secondY);
  ...
  bool _detectOutsideLabelCollision(
      num labelY, bool labelLeftOfChart, num previousOutsideLabelY, bool previousLabelLeftOfChart) {
    bool collides = false;
  ...
  double _drawLeaderLine(
    ...
    final leaderLineTailPoint = new Point<double>(labelPoint.x + tailX, labelPoint.y);

    final centerRadius = radius - leaderLineStyleSpec.length / 2;
    final leaderLineStartPoint = new Point<double>(
        arcCenterPoint.x + centerRadius * cos(trueCenterAngle ?? centerAngle),
        arcCenterPoint.y + centerRadius * sin(trueCenterAngle ?? centerAngle));

    canvas.drawLine(points: [
      leaderLineStartPoint,
      labelPoint,
      leaderLineTailPoint,
    ], stroke: leaderLineStyleSpec.color, strokeWidthPx: leaderLineStyleSpec.thickness);
